...
$scriptPath = "$PSScriptRoot\AddKeyVaultAccessPolicy.ps1"
$templateFile = "$PSScriptRoot\azuredeploy.json"

Write-Host "$scriptPath $templateFile"
...
try {
    if ($input_pwsh)
    {
        ...
        if ($scriptType -eq "InlineScript") {
            ...
            $contents += ". '$PSScriptRoot\$("$scriptName".Replace("'", "''"))' -resourceGroup $resourceGroupName -keyVaultName $keyVaultName -templateFile $templateFile".Trim()
        }
    ...
    else
    {
        ...
        $scriptCommand = "& '$($scriptPath.Replace("'", "''"))'  -resourceGroup $resourceGroupName -keyVaultName $keyVaultName -templateFile $templateFile"
